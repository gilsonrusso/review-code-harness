...
export async function publishReview(
  ...
  validator?: DiffCoordinateValidator,
  dryRun = false,
  commitSha?: string
): Promise<void> {
  ...
  let existingSummaryReviewId: number | undefined;
  try {
  ...
  const filteredInlineComments: any[] = [];
  if (inlineComments.length > 0) {
    try {
      ...
      for (const comment of inlineComments) {
        const isDuplicate = existingInlines.some(exist =>
          exist.path === comment.path &&
