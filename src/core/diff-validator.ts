 ...
 */
export function parsePatchToLines(patch: string): Set<number> {
...
export class DiffCoordinateValidator {
  ...
  private gitMetadata?: { owner: string; repo: string; pullNumber?: number; commitSha: string };
  private token?: string;
  private changedLinesMap = new Map<string, Set<number>>();
  ...
  constructor(
    ...
    gitMetadata?: { owner: string; repo: string; pullNumber?: number; commitSha: string },
    token?: string
  ) {
   ...
   */
  private parseRawDiff(rawDiff: string): void {
