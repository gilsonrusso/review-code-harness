...
import { Skill, Diff, DiffFile } from '../models/types.js';
 ...
 */
export function parseRawDiff(rawDiff: string): Diff {
  ...
  for (const part of parts) {
    ...
    else if (deletedFile) status = 'deleted';
 ...
 */
export async function loadDiff(workspaceRoot: string, baseRef?: string): Promise<Diff> {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Falha ao obter o diff do git. Erro original: ${error.message}. Fallbacks falharam: ${err.message}`
        );
      }
    }
  }
