...
describe('loadConfig', () => {
  ...
  it('deve retornar valores padrão se o arquivo não existe', async () => {
    const configPath = path.join(tempConfigDir, 'nonexistent.yml');
    ...
    // Verifica que o arquivo padrão foi criado fisicamente no disco
    const fileCreated = await fs.access(configPath).then(() => true).catch(() => false);
