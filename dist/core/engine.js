...
export async function runReviewEngine(options = {}) {
    ...
    const rawOutput = await adapter.run(instructions, config.review.timeoutSeconds, config.review.maxRetries);
    ...
    await publishReview(finalReviewResult, config.output.mode, validator, options.dryRun || process.env.NODE_ENV === "test", commitSha);
